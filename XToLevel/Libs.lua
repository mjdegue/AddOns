...
function XToLevel.Lib:ShowBattlegroundData()
	return ((XToLevel.db.profile.ldb.tooltip.showBGInfo and XToLevel.Player.level >= 10) and
		(XToLevel.Player.level < XToLevel.Player.maxLevel or (#XToLevel.db.char.data.bgList) > 0))
end

function XToLevel.Lib:ShowDungeonData()
	return ((XToLevel.db.profile.ldb.tooltip.showDungeonInfo) and
		(XToLevel.Player.level < XToLevel.Player.maxLevel or (#XToLevel.db.char.data.dungeonList) > 0))
end
...
function XToLevel.Lib:MobXP(charLevel, mobLevel, mobClassification)
	...
	if mobClassIndex == nil then
		console:log(
			"Lib->MobXP: Invalid mobClassification passed. Defaulting to 'normal'. ('" .. tostring(mobClassification) .. "')"
		)
		mobClassIndex = 1
	end

	if type(charLevel) ~= "number" then
	...
	if
		type(XToLevel.db.char.data.npcXP[charLevel]) == "table" and
			type(XToLevel.db.char.data.npcXP[charLevel][mobLevel]) == "table" and
			type(XToLevel.db.char.data.npcXP[charLevel][mobLevel][mobClassIndex]) == "table" and
			#XToLevel.db.char.data.npcXP[charLevel][mobLevel][mobClassIndex] > 0
	 then
		local high = 0
...
function XToLevel.Lib:GetProgressColor(pro)
	if pro <= 0 then
		pro = 1
	end -- 0 doesn't play well with the formulas. CBA to fix that so I just bypass it like so.
	if pro > 100 then
		pro = 100
	end
	if self.progressColor.pro ~= pro then
		...
		self.progressColor.hex =
			XToLevel.Lib:DecToHex(self.progressColor.rgb.r, 2) ..
			XToLevel.Lib:DecToHex(self.progressColor.rgb.g, 2) .. XToLevel.Lib:DecToHex(self.progressColor.rgb.b, 2)
	end
