...
function XToLevel.Player:Initialize()
    ...
    if XToLevel.db.profile.timer.enabled then
        self.timerHandler =
            XToLevel.timer:ScheduleRepeatingTimer(XToLevel.Player.TriggerTimerUpdate, self.xpPerSecTimeout)
    end
end
...
function XToLevel.Player:UpdateTimer()
    ...
    if useMode == 1 then
        if
            type(XToLevel.db.char.data.timer.start) == "number" and type(XToLevel.db.char.data.timer.total) == "number" and
                XToLevel.db.char.data.timer.total > 0
         then
            XToLevel.db.char.data.timer.xpPerSec =
                XToLevel.db.char.data.timer.total /
                (XToLevel.db.char.data.timer.lastUpdated - XToLevel.db.char.data.timer.start)
    ...
    if useMode == 2 then
        if
            type(self.timePlayedLevel) == "number" and
                (self.timePlayedLevel + (XToLevel.db.char.data.timer.lastUpdated - self.timePlayedUpdated)) > 0
         then
            local xpPerSec =
                self.currentXP /
                (self.timePlayedLevel + (XToLevel.db.char.data.timer.lastUpdated - self.timePlayedUpdated))
            if xpPerSec > 0 then
...
function XToLevel.Player:GetTimerData()
    ...
    elseif
        XToLevel.db.profile.timer.mode == 1 and XToLevel.db.char.data.timer.xpPerSec ~= nil and
            tonumber(XToLevel.db.char.data.timer.xpPerSec) > 0
     then
        mode = 1
        warning = 1
        timePlayed = GetTime() - XToLevel.db.char.data.timer.start
...
function XToLevel.Player:AddGathering(action, target, xp)
    if (action == nil or target == nil or xp == nil) then
        console:log(
            "Attempt to add invalid gathering data: " ..
                tostring(action) .. ", " .. tostring(target) .. ", " .. tostring(xp)
        )
        return nil
    else
        ...
        for i, v in ipairs(XToLevel.db.char.data.gathering[action]) do
            if v["target"] == target and v["level"] == XToLevel.Player.level and v["zoneID"] == zoneID then
                incremented = true
                XToLevel.db.char.data.gathering[action][i]["count"] =
                    XToLevel.db.char.data.gathering[action][i]["count"] + 1
...
function XToLevel.Player:GetAverageDungeonXP()
    if self.dungeonAverage == nil then
        if
            (#XToLevel.db.char.data.dungeonList > 0) and
                not ((#XToLevel.db.char.data.dungeonList == 1) and XToLevel.db.char.data.dungeonList[1].inProgress)
         then
            local total = 0
...
function XToLevel.Player:GetLatestDungeonDetails()
    if #XToLevel.db.char.data.dungeonList > 0 then
        ...
        self.latestDungeonData.otherXP =
            XToLevel.db.char.data.dungeonList[1].totalXP - XToLevel.db.char.data.dungeonList[1].killTotal
        if self.latestDungeonData.killCount > 0 then
            self.latestDungeonData.xpPerKill =
                XToLevel.Lib:round(XToLevel.db.char.data.dungeonList[1].killTotal / self.latestDungeonData.killCount, 0)
        end
