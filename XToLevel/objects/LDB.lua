...
function XToLevel.LDB:Initialize()
    if
        not XToLevel.db.profile.ldb.enabled or
            (XToLevel.Player:GetMaxLevel() == XToLevel.Player.level and XToLevel.Player:GetClass() ~= "HUNTER")
     then
        return
    end
    ...
    local iconName =
        (UnitFactionGroup("player") == "Alliance") and "INV_Jewelry_TrinketPVP_01" or "INV_Jewelry_TrinketPVP_02"
    ...
    if XToLevel.db.profile.ldb.showText then
        ...
        if XToLevel.Player.level < XToLevel.Player:GetMaxLevel() then
            ...
            pattern =
                ...
                pattern,
                "%$%$kills%$%$",
                (XToLevel.Lib:round(XToLevel.Player:GetAverageKillsRemaining()) or "~")
            )
            pattern =
                string.gsub(
                pattern,
                "%$%$quests%$%$",
                (XToLevel.Lib:round(XToLevel.Player:GetAverageQuestsRemaining()) or "~")
            )
            pattern =
                string.gsub(
                pattern,
                "%$%$dungeons%$%$",
                (XToLevel.Lib:round(XToLevel.Player:GetAverageDungeonsRemaining()) or "~")
            )
            ...
            if XToLevel.db.profile.ldb.text.xpAsBars then
                pattern =
                    ...
                    pattern,
                    "%$%$xp%$%$",
                    tostring(XToLevel.Player:GetProgressAsBars()) .. " " .. L["Bars"]
                )
            else
            ...
            local xpnum =
                XToLevel.db.profile.ldb.text.xpCountdown and XToLevel.Player:GetXpRemaining() or
                XToLevel.Player.currentXP
            xpnum =
                XToLevel.db.profile.ldb.text.xpnumFormat and XToLevel.Lib:ShrinkNumber(xpnum) or
                XToLevel.Lib:round(xpnum)
            ...
            pattern =
                ...
                pattern,
                "%$%$bgs%$%$",
                (XToLevel.Lib:round(XToLevel.Player:GetAverageBGsRemaining()) or "~")
            )
            pattern =
                string.gsub(
                pattern,
                "%$%$bgo%$%$",
                (XToLevel.Lib:round(XToLevel.Player:GetAverageBGObjectivesRemaining()) or "~")
            )
            ...
            if XToLevel.db.profile.ldb.text.xpnumFormat then
                pattern =
                    string.gsub(pattern, "%$%$rested%$%$", (XToLevel.Lib:ShrinkNumber(XToLevel.Player.restedXP) or "~"))
            else
                pattern = string.gsub(pattern, "%$%$rested%$%$", (XToLevel.Lib:round(XToLevel.Player.restedXP) or "~"))
            end
            if XToLevel.db.profile.ldb.text.xpAsBars then
                local restedbars =
                    XToLevel.Lib:round(XToLevel.Lib:round(XToLevel.Player:GetRestedPercentage()) / 5, 0, false)
                ...
                pattern =
                    ...
                    pattern,
                    "%$%$restedp%$%$",
                    (XToLevel.Lib:round(XToLevel.Player:GetRestedPercentage(1)) .. "%%" or "~")
                )
            end
