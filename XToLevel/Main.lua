...
XToLevel = {}
XToLevel.version = "7.0.3_46"
XToLevel.releaseDate = "2016-07-25T05:27:39Z"
...
XToLevel.frame:SetScript(
    "OnEvent",
    function(self, ...)
        XToLevel:MainOnEvent(...)
    end
)
    ...
    if not addonTable.SetLocale(XToLevel.db.profile.general.displayLocale) then
        console:log(
            "Attempted to load unknown locale '" ..
                tostring(XToLevel.db.profile.general.displayLocale) .. "'. Falling back on 'enUS'."
        )
        XToLevel.db.profile.general.displayLocale = "enUS"
        if not addonTable.SetLocale("enUS") then
            XToLevel.Messages:Print(
                "|cFFaaaaaaXToLevel - |r|cFFFF5533Fatal error:|r Locale files not found. (Try re-installing the addon.)"
            )
            return
        end
    end
    ...
    if mobName ~= nil then
        if XToLevel.Player:IsBattlegroundInProgress() then
            ...
            if XToLevel.db.profile.messages.playerFloating or XToLevel.db.profile.messages.playerChat then
                ...
                if killsRequired > 0 then
                    XToLevel.Messages.Floating:PrintKill(
                        mobName,
                        ceil(killsRequired / ((XToLevel.Lib:IsRafApplied() and 3) or 1))
                    )
        ...
        if XToLevel.Player:IsBattlegroundInProgress() then
            ...
            if isQuest then
                ...
                if XToLevel.db.profile.messages.playerFloating or XToLevel.db.profile.messages.playerChat then
                    ...
                    if questsRequired > 0 then
                        XToLevel.Messages.Floating:PrintQuest(
                            ceil(questsRequired / ((XToLevel.Lib:IsRafApplied() and 3) or 1))
                        )
            ...
            elseif isArch then
                ...
                if
                    XToLevel.gatheringTarget ~= nil and XToLevel.gatheringTime ~= nil and
                        GetTime() - XToLevel.gatheringTime < 5
                 then
                        ...
                        tostring(row["target"]) ..
                            ", l:" ..
                                tostring(row["level"]) ..
                                    ", xp:" ..
                                        tostring(row["xp"]) ..
                                            ", z:" .. tostring(row["zoneID"]) .. ", x" .. tostring(row["count"])
                )
            end
        end
